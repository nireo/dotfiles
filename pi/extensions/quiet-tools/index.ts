...
function createQuietToolDefinitions(cwd: string, enabled: boolean): ToolDefinition[] {
	...
	return enabled
		? baseDefinitions.map((tool) => QUIET_CALL_TOOL_NAMES.has(tool.name) ? createQuietToolDefinition(tool) : tool)
		: baseDefinitions;
}
...
export default function quietToolsExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("quiet-tools", {
		description: "Toggle compact collapsed invocations for built-in tool rows",
