...
export default function readOnlyModeExtension(pi: ExtensionAPI) {
	...
	function enableReadOnlyMode(ctx: ExtensionContext): void {
		if (enabled) {
			...
			if (ctx.hasUI) ctx.ui.notify("Read-only mode is already enabled.", "info");
			return;
		}
		...
		const tools = getReadOnlyToolNames(pi).join(", ");
		if (ctx.hasUI) ctx.ui.notify(`Read-only mode enabled. Tools: ${tools || "(none)"}.`, "info");
	}

	function disableReadOnlyMode(ctx: ExtensionContext): void {
		if (!enabled) {
			updateUi(pi, ctx, enabled);
			if (ctx.hasUI) ctx.ui.notify("Read-only mode is already disabled.", "info");
			return;
		}
		...
		updateUi(pi, ctx, enabled);
		if (ctx.hasUI) ctx.ui.notify("Read-only mode disabled. Previous tool access restored.", "info");
	}
		...
		handler: async (args, ctx) => {
			...
			switch (action) {
				...
				case "status": {
					...
					const tools = getReadOnlyToolNames(pi).join(", ");
					ctx.ui.notify(
						enabled ? `Read-only mode is ON. Allowed tools: ${tools || "(none)"}.` : "Read-only mode is OFF.",
						"info",
					);
					return;
				}
