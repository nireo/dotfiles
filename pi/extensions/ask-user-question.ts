...
export default function askUserQuestion(pi: ExtensionAPI) {
	pi.registerTool({
		...
		description:
			"Ask the user a single question and pause execution until they answer. Use this when requirements are ambiguous, user preferences are needed, a decision would materially affect implementation, or confirmation is needed before proceeding. Ask exactly one question per tool call, and prefer multiple separate tool calls over bundling unrelated questions together.",
		promptSnippet:
		...
		promptGuidelines: [
			"Use ask_user_question for exactly one question per tool call.",
			"If multiple questions are needed, make multiple separate ask_user_question tool calls instead of combining them into one prompt.",
			'When ask_user_question is called with options, users will always be able to select "Other" to provide custom text input.',
			"Use ask_user_question with multiSelect: true only when you need multiple answers to the same question.",
			'If recommending a specific option in ask_user_question, make it the first option in the list and add "(Recommended)" at the end of the label.',
